...
class TestPositive:
    ...
    def test_step2(self, clear_folders, make_files):
        # test2
        res = []
        res.append(checkout("cd {0}; 7z a -t{2} {1}/arx".format(data["folder_in"], data["folder_out"], data["type"]), "Everything is Ok"))
        res.append(checkout("cd {0}; 7z e arx.{2} -o{1} -y".format(data["folder_out"], data["folder_ext"], data["type"]), "Everything is Ok"))
    ...
    def test_step3(self):
        # test3
        assert checkout("cd {0}; 7z t -t{1} arx.{1}".format(data["folder_out"], data["type"]), "Everything is Ok"), "test3 FAIL"

    def test_step4(self):
        # test4
        assert checkout("cd {0}; 7z u arx2.{1}".format(data["folder_in"], data["type"]), "Everything is Ok"), "test4 FAIL"
    ...
    def test_step6(self, clear_folders, make_files, make_subfolder):
        ...
        res = []
        res.append(checkout("cd {0}; 7z a -t{2} {1}/arx".format(data["folder_in"], data["folder_out"], data["type"]), "Everything is Ok"))
        res.append(checkout("cd {0}; 7z x arx.{2} -o{1} -y".format(data["folder_out"], data["folder_ext2"], data["type"]), "Everything is Ok"))
        for i in make_files:
